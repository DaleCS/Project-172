...
import { getCurrentUserData, getHeaders, getURI } from "../auth";

export const fetchTodoLists = async (setLoading) => {
  setLoading(true);
  try {
    const res = await axios.get(
      `${getURI()}/api/todolists/allLists?user_id=${
...
export const fetchTodoListEntries = async (todoListId, setLoading) => {
  ...
  try {
    const res = await axios.get(
      `${getURI()}/api/todolists/allEntries?todoListID=${todoListId}`,
      getHeaders(true)
    );
...
export const updateTodoList = async (
  ...
  try {
    ...
    await axios.post(
      `${getURI()}/api/todolists/updateListTitle`,
      body,
      getHeaders(true)
    );
    setTodoLists(await fetchTodoLists(setLoading));
...
export const deleteTodoList = async (
  ...
  try {
    await axios.post(
      `${getURI()}/api/todolists/deleteList?todoListID=${todoListId}`,
...
export const updateTodoListEntry = async (
  ...
  try {
    ...
    await axios.post(
      `${getURI()}/api/todolists/updateEntryTitle`,
      body,
      getHeaders(true)
    );
    setTodoListEntries(
...
export const deleteTodoListEntry = async (
  ...
  try {
    await axios.post(
      `${getURI()}/api/todolists/deleteListEntry?entryID=${entryId}`,
